...
# Hardcoded tool primitives (T_shape as example)
TOOL_PARAMS = {
    "is_composite": True,
    "tool_dims": [
        # [0.03, 0.03, 0.1],  # stem_dims
        [0.15, 0.03, 0.03]   # bar_dims
    ],
    "tool_offsets": [
        # [0, 0, 0.05],        # stem offset
        [0.05, 0, 0.05]        # bar offset (stem_dims[2] + bar_dims[2]/2 - 0.01)
    ],
    "tool_quats": [
        # [1.0, 0.0, 0.0, 0.0],  # identity quaternion for stem
        [1.0, 0.0, 0.0, 0.0]   # identity quaternion for bar
    ],
...
class ReactiveController:
    ...
    def sample_robot_points(self, config, num_points):
        """Sample robot points with tool support"""
        device = config.device
        dtype = config.dtype

        if self.tool_params["tool_num_primitives"] > 0:
            # For composite tools
            if self.tool_params["is_composite"]:
                tool_dims = self.tool_params["tool_dims"]
                tool_offsets = self.tool_params["tool_offsets"]
                tool_quats = self.tool_params["tool_quats"]
            else:
            ...
            # No tool - use standard sampling
            return self.fk_sampler.sample(config, num_points)

    def sample_target_points(self, pose, num_points):
        """Sample target points with tool support"""
        device = pose.device
        dtype = pose.dtype
        ...
        if self.tool_params["tool_num_primitives"] > 0:
            ...
            return self.fk_sampler.sample_composite_end_effector(
                pose,
                tool_dims_tensor,
                tool_offsets_tensor,
                tool_quats_tensor,
    ...
    def get_next_action(
        ...
        # Sample robot points with tool support
        robot_points = self.sample_robot_points(q, NUM_ROBOT_POINTS).squeeze(0)
        rospy.loginfo("Sampled robot points shape: {}".format(robot_points.shape))

        # Get target points with tool support
        target_points = self.target_point_cloud(target_pose).squeeze()
        rospy.loginfo("Sampled target points shape: {}".format(target_points.shape))
        ...
        rospy.loginfo("Model action output: {}".format(action))
...
class ReactiveControllerNode:
    ...
    def send_joint_command(self, joint_positions: np.ndarray, joint_velocities: np.ndarray, joint_accelerations: np.ndarray):
        ...
        with self.control_lock:
            ...
            base_duration = 0.06  # Minimum duration for very small movements
            max_allowed_duration = 0.6  # Maximum duration for safety
            ...
            if self.target_reached:
                duration = 4  # Fixed duration when maintaining position
