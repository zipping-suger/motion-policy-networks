...
class ReactiveController:
    ...
    def get_next_action(
        ...
        Updated to use model like planning_node.py
        ...
        # Create point cloud exactly like planning_node.py
        ...
        # Get action from model - USE SAME CALL AS PLANNING_NODE
        # The model in planning_node.py is called as: q_norm + self.mdl(point_cloud, q_norm)
        delta_q = self.mdl(point_cloud, q_norm)
        q_next_norm = torch.clamp(q_norm + delta_q, min=-1, max=1)
...
class ReactiveControllerNode:
    def __init__(self):
        ...
        self.control_step_count = 0  # Track number of control steps
    ...
    def stop_control_callback(self, msg):
        ...
        if msg.data:
            ...
            with self.control_lock:
                ...
                self.control_step_count = 0  # Reset step counter
